...
class songmanage{
    ...
    function mupload(){
        $_FILES['file'];
        if(is_uploaded_file($_FILES['file']['tmp_name'])){
            if(!file_exists('Public/music/upload')){
                mkdir('Public/music/upload');
            }
            $data=date('y-m-d');
            if(!file_exists('Public/music/upload/'.$data)){
                mkdir('Public/music/upload/'.$data);
            }
            $type=explode('/',$_FILES['file']['type'])[1];
            $path='Public/music/upload/'.$data.'/'.time().rand(1,100).'.'.$type;
            if(move_uploaded_file($_FILES['file']['tmp_name'],$path)){
                echo '/php/ktv/'.$path;
            }
        }
